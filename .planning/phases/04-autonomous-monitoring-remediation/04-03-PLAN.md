...
must_haves:
  truths:
    - "The dashboard shows a live activity feed with timestamped Jarvis observations and actions updating in real-time"
    - "Monitor-sourced events are visually distinguished from user-sourced events in the ActivityFeed"
    - "A visible kill switch toggle on the dashboard disables all autonomous actions when toggled"
    - "Kill switch state is displayed prominently and toggleable in the TopBar"
    - "The activity feed shows the full remediation sequence: detected -> acting -> verified -> resolved"
  artifacts:
