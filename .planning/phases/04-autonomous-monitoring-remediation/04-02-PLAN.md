...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/api/routes.ts"
      provides: "GET /api/monitor/status and PUT /api/monitor/killswitch endpoints, setupMonitorRoutes accepting eventsNs"
  ...
  key_links:
    ...
    - from: "src/index.ts"
      ...
      via: "setupMonitorRoutes(router, eventsNs) dependency injection"
      pattern: "setupMonitorRoutes\\(router.*eventsNs\\)"
---
...
@jarvis-backend/src/index.ts (wire setupMonitorRoutes with eventsNs dependency injection)
</context>
...
<task type="auto">
  ...
  <action>
    **1. Create `src/monitor/guardrails.ts`**:
    ...
    - `isNodeBeingRemediated(node: string): boolean` -- check if a specific node is in activeRemediations (for blast radius verification)
    ...
    **2. Create `src/monitor/runbooks.ts`**:
    ...
    import { checkGuardrails, recordAttempt, markRemediationActive, markRemediationComplete, getAttemptCount } from './guardrails.js';
    ...
    **Runbook definitions** (array of Runbook objects):
    ...
    - `executeRunbook(incident: Incident, eventsNs: Namespace): Promise<void>` -- the core remediation pipeline:
      ...
      4. If guardrails block:
         - If reason includes 'Rate limit' -> call escalation: send escalation email, save autonomy action with result='escalated', record the attempt count. **After escalation, NO further attempts should be made for this incident key** -- the rate limiter naturally enforces this since attempts >= 3.
         - Save event describing the block. Emit to eventsNs with `source: 'monitor'`
         - Return
      5. Record attempt: recordAttempt(incident.key)
      6. Mark remediation active: markRemediationActive(incident.node)
      7. Emit "remediation starting" event to eventsNs with `source: 'monitor'`
      ...
      12. Emit result event to eventsNs (resolved or failed) with `source: 'monitor'`
      13. If success: send remediation email (success report)
      14. If failure: save event, check if attempt count now >= 3, if so escalate -- send escalation email AND save action with result='escalated' to ensure both email AND attempt-blocking happen together
    ...
    **3. Create `src/monitor/reporter.ts`**:
    ...
    ```typescript
    import { execOnNode } from '../clients/ssh.ts';
  ...
  <verify>
    ...
    - **Blast radius test:** Call `markRemediationActive('nodeA')`, then call `checkGuardrails('test:key', 'nodeB', 3)` -- must return `{ allowed: false, reason: 'Another remediation is in progress (blast radius control)' }`. Then call `markRemediationComplete('nodeA')` and verify `checkGuardrails('test:key', 'nodeB', 3)` now returns `{ allowed: true }`. Run this test inline during development to confirm blast radius enforcement.
  </verify>
  <done>
    ...
    - Blast radius verified: concurrent remediation on different nodes is blocked
...
<task type="auto">
  <name>Task 2: Wire runbooks into pollers and add monitor REST API with dependency injection</name>
  <files>
    src/monitor/poller.ts
    src/monitor/index.ts
    src/api/routes.ts
  ...
  <action>
    ...
    **3. Refactor monitor route setup in `src/api/routes.ts` to use dependency injection (NO circular imports):**

    **DO NOT** use `await import('../index.js')` to get eventsNs. This creates a fragile circular dependency.

    Instead, create a separate exported function that receives eventsNs as a parameter:

    ```typescript
    // At the end of routes.ts, add:
    export function setupMonitorRoutes(router: Router, eventsNs: Namespace): void {
      ...
      router.put('/api/monitor/killswitch', (req: Request, res: Response) => {
        ...
        // eventsNs received via parameter -- no circular import
        eventsNs.emit('event', {
          id: randomUUID(),
          type: 'status',
          severity: active ? 'warning' : 'info',
    ...
    **4. Wire setupMonitorRoutes in `src/index.ts`:**

    After the Socket.IO namespaces are created (where eventsNs is available), call:

    ```typescript
    import { setupMonitorRoutes } from './api/routes.js';

    // After socket.io setup, after existing route setup:
    setupMonitorRoutes(router, eventsNs);
    ```

    This passes eventsNs into the routes via parameter injection, avoiding any circular `import('../index.js')` pattern.

    **NOTE on crypto:** Use `import { randomUUID } from 'node:crypto'` at the top of routes.ts for the kill switch event ID.
  </action>
  <verify>
    - `npx tsc --noEmit` passes with no type errors
    - **No circular imports:** `grep -r "import.*from.*'\\.\\.\/index" src/api/routes.ts` returns zero matches -- routes.ts does NOT import from index.ts
  ...
  <done>
    ...
    - eventsNs passed to routes via dependency injection (setupMonitorRoutes function) -- NO circular imports
...
2. **No circular imports:** Confirm routes.ts does not import from index.ts: `grep -r "from.*['\"]\\.\\.\/index" src/api/routes.ts` returns nothing
3. End-to-end test: Set kill switch active, verify all autonomous actions are blocked
4. End-to-end test: Set kill switch inactive, simulate a detection (if a VM happens to transition), verify runbook executes
5. API test: GET /api/monitor/status returns correct state
6. API test: PUT /api/monitor/killswitch toggles the state and emits events
7. API test: GET /api/monitor/actions returns audit log entries
8. Rate limiter test: After 3 attempts for same incident key, 4th attempt is blocked
9. Blast radius test: While one remediation is active on node A, a remediation attempt on node B is blocked with reason 'Another remediation is in progress (blast radius control)'
10. **Escalation e2e test:** Simulate 3 failed remediation attempts for the same incident key within 1 hour:
    - Record 3 attempts via recordAttempt() with the same incident key
    - On the 3rd failure, verify:
      a. sendEscalationEmail is called (check console logs for SSH command output or mock)
      b. The autonomy action is saved with result='escalated'
      c. A 4th attempt for the same key is blocked by checkGuardrails with reason 'Rate limit exceeded'
    - This confirms the full escalation path: 3 failures -> email sent -> further attempts blocked
</verification>
...
- Escalation emails are sent after 3 failed attempts AND further attempts are blocked
- Every autonomous action is recorded in the autonomy_actions audit log
- All errors are caught -- the monitor never crashes the backend
- No circular imports between routes.ts and index.ts
</success_criteria>
