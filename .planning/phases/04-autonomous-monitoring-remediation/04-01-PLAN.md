...
must_haves:
  truths:
    ...
    - "All emitted events include source: 'monitor' field matching the JarvisEvent interface"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/monitor/poller.ts"
      to: "JarvisEvent interface"
      via: "emitted events include source: 'monitor' field"
      pattern: "source:\\s*['\"]monitor['\"]"
---
...
<task type="auto">
  ...
  <action>
    ...
    **3. Create `src/monitor/poller.ts`**:
    ...
    - `pollCritical(eventsNs, stateTracker)`: Poll nodes (getClusterResources('node')) and VMs (getClusterResources('vm')). Run stateTracker.updateNodes() and stateTracker.updateVMs(). For each StateChange detected:
      ...
      - Emit to eventsNs as `'event'` with **explicit `source: 'monitor'` field** matching the JarvisEvent interface: `{ id: crypto.randomUUID(), type: 'alert', severity, title, message, node, source: 'monitor', timestamp: new Date().toISOString() }`
    ...
    - `pollImportant(eventsNs, thresholdEvaluator)`: Poll nodes. Run evaluateThresholds(). For new threshold violations:
      - Call memoryStore.saveEvent() with type='alert', severity from violation, source='monitor'
      - Emit to eventsNs as `'event'` with **explicit `source: 'monitor'` field**: `{ id: crypto.randomUUID(), type: 'alert', severity, title, message, node, source: 'monitor', timestamp: new Date().toISOString() }`
    ...
    **CRITICAL -- source field contract:** Every event emitted to eventsNs MUST include `source: 'monitor'`. This field is required by the frontend JarvisEvent interface (Plan 03) for source-based filtering and visual distinction. Without it, monitor events will be indistinguishable from user events in the ActivityFeed. The `source` field must be a string literal `'monitor'`, not a variable or computed value.
  ...
  <verify>
    ...
    - **Source field check:** Grep poller.ts to confirm all `eventsNs.emit('event', ...)` calls include `source: 'monitor'` in the payload: `grep -n "source.*monitor" src/monitor/poller.ts` must return at least 2 matches (one for pollCritical, one for pollImportant)
  </verify>
  <done>
    ...
    - All emitted events include `source: 'monitor'` field matching JarvisEvent interface
...
7. **Source field validation:** `grep -c "source.*'monitor'" src/monitor/poller.ts` returns >= 2, confirming all event emissions include the source field
</verification>
...
- All emitted events include `source: 'monitor'` field for frontend compatibility
</success_criteria>
