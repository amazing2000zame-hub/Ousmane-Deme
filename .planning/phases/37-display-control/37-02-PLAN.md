...
- **Handler logic:**
  ...
  - For `show_camera`: POST to `/display/show` with `{"url": "http://192.168.1.65:5000/api/front_door"}` (or the appropriate Frigate/go2rtc URL for the named camera). Map camera names: `front_door` -> `http://192.168.1.65:8085/api/ws?src=front_door` (go2rtc WebRTC), `side_house` -> `http://192.168.1.65:8085/api/ws?src=side_house`. If camera name is unrecognized, return error listing available cameras.
