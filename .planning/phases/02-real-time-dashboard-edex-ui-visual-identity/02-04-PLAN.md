...
<task type="auto">
  ...
  **components/shared/GlowBorder.tsx:**
  - Props: children, color? ('amber' | 'cyan' | 'green' | 'red'), intensity? ('low' | 'medium' | 'high'), className?
  ...
  - Only renders the glow effect if the current visual mode has glowEffects enabled (read from useUIStore -> check VISUAL_MODES[mode].glowEffects)
  ...
  **Wire into Dashboard.tsx:**
  ...
  - Left column: NodeGrid + VMList + StoragePanel (stacked vertically with overflow-y-auto)
  - Center column: CenterDisplay
  - Right column: keep placeholder (will be filled by Plan 02-05)
  </action>
  <verify>
  ...
  <done>
  Left column is complete with NodeGrid, VMList, and StoragePanel stacked vertically. Center column shows ActivityFeed with real-time events. Storage usage is visualized with bars and threshold coloring. The dashboard now shows all cluster infrastructure data.
  </done>
</task>
...
6. StoragePanel shows usage bars for all storage pools
7. ActivityFeed renders events from store
8. CenterDisplay wraps ActivityFeed in PanelFrame
9. Dashboard.tsx renders all left column panels and center display
10. GlowBorder respects visual mode settings
</verification>

<success_criteria>
The left column is fully populated with node health (from Plan 02-03), VM/CT controls, and storage overview. The center column shows a real-time activity feed. Users can start VMs with one click and stop/restart with confirmation. All data flows from the backend through Socket.IO to Zustand to rendered components.
</success_criteria>
