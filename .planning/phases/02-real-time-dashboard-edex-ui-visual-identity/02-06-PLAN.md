...
<task type="auto">
  <name>Task 3: Automated data timing verification</name>
  <files>
    (no new files -- this task runs automated checks)
  </files>
  <action>
  Before the human checkpoint, run automated verification to confirm the Success Criterion 1 timing requirement (10-15s data updates).

  **Verification script (run inline via node/tsx):**

  Create and run a temporary verification script that:
  1. Connects to the backend Socket.IO /cluster namespace with a valid JWT token
  2. Listens for 'nodes', 'vms', 'storage', and 'quorum' events
  3. Records timestamps for each event type received
  4. Waits for at least 45 seconds (to capture multiple update cycles)
  5. Calculates the interval between consecutive events of each type
  6. Asserts:
     - 'nodes' events arrive every 8-12 seconds (target: 10s)
     - 'vms' events arrive every 13-17 seconds (target: 15s)
     - 'storage' events arrive every 28-32 seconds (target: 30s)
     - At least one event of each type was received
  7. Prints a summary table of observed intervals
  8. Exits with code 0 on success, code 1 on failure
  ...
  **Implementation approach:**
  ...
  TOKEN=$(curl -s -X POST http://localhost:4000/api/auth/login \
    ...
    -d '{"password":"'$JARVIS_PASSWORD'"}' | node -e "process.stdin.on('data',d=>process.stdout.write(JSON.parse(d).token))")

  # Run timing verification
  npx tsx -e "
  import { io } from 'socket.io-client';
  const token = '${TOKEN}';
  const socket = io('http://localhost:4000/cluster', { auth: { token }, autoConnect: true });
  const timestamps: Record<string, number[]> = { nodes: [], vms: [], storage: [], quorum: [] };
  for (const key of Object.keys(timestamps)) {
    socket.on(key, () => { timestamps[key].push(Date.now()); });
  }
  setTimeout(() => {
    socket.disconnect();
    let allPass = true;
    const expected: Record<string, [number, number]> = { nodes: [8000, 12000], vms: [13000, 17000], storage: [28000, 32000] };
    for (const [key, [min, max]] of Object.entries(expected)) {
      const ts = timestamps[key];
      if (ts.length < 2) { console.log('FAIL: ' + key + ' - fewer than 2 events received'); allPass = false; continue; }
      const intervals = ts.slice(1).map((t, i) => t - ts[i]);
      const avg = intervals.reduce((a, b) => a + b, 0) / intervals.length;
      const pass = avg >= min && avg <= max;
      console.log((pass ? 'PASS' : 'FAIL') + ': ' + key + ' avg interval=' + Math.round(avg) + 'ms (expected ' + min + '-' + max + 'ms, ' + intervals.length + ' intervals)');
      if (!pass) allPass = false;
    }
    if (timestamps.quorum.length < 1) { console.log('FAIL: quorum - no events received'); allPass = false; }
    else { console.log('PASS: quorum - ' + timestamps.quorum.length + ' events received'); }
  ...
  **Important notes:**
  - This task requires the backend to be running. Start it before running the verification if not already running.
  - The backend must have valid Proxmox API tokens configured to actually emit data.
  - If the backend is not running or tokens are not configured, document this as a prerequisite that must be met before running the automated check, and note it for the human checkpoint.
  - If the verification fails, investigate whether the emitter intervals are misconfigured or if there is a connection issue.
  </action>
  <verify>
  The verification script exits with code 0, confirming:
  - 'nodes' events arrive approximately every 10 seconds
  - 'vms' events arrive approximately every 15 seconds
  - 'storage' events arrive approximately every 30 seconds
  - 'quorum' events are received at least once
  If the backend is not running, note this for the human checkpoint.
  </verify>
  <done>
  Automated timing verification confirms that WebSocket data events arrive within the expected intervals (10s nodes, 15s VMs, 30s storage), satisfying Success Criterion 1.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  - Automated timing verification (Task 3 results)
  </what-built>
  <how-to-verify>
  ...
  **Check Task 3 automated timing results first.** If they passed, Success Criterion 1 (10-15s data updates) is verified. If they failed or could not run, manually verify data updates by watching the node cards for ~30 seconds and confirming values change.
...
12. Automated timing verification confirms data arrives within expected intervals
</verification>

<success_criteria>
The dashboard has an unmistakable Iron Man / eDEX-UI sci-fi aesthetic. Boot sequence sets the cinematic tone. Ambient animations make the screen feel alive (scan lines, grid drift, data pulse). Three visual modes provide different levels of visual intensity. Keyboard shortcuts enable mouse-free navigation. The layout works on dedicated displays and desktop browsers. Automated verification confirms data timing requirements. Human verification confirms the visual quality and interactive functionality meet the product vision.
</success_criteria>
